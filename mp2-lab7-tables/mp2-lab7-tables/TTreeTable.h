...
/* Запись в дереве - запись и указатели на левое и правое звенья */
struct TTreeNode
{
	TRecord rec;
	TTreeNode* pLeft, * pRight;

	//Показатель балансировки звена (нужен для подсчёта балансировки в TBalTreeTable)
	int Bal;
...
protected:
	...
	TTreeNode* pRoot;
	//Указатель на предыдущий и текущий элементы
...
//(в неё передаётся указатель на удаляемое звено)
...
{
	//Если есть возможность выделить память под новое звено - таблица не полна.
	try
	{
...
{
	//Установка начального положения - корень
	pPrev = nullptr;
	pCurr = pRoot;
	...
	{
		Eff++;
		//Запись с таким ключом найдена; pCurr и pPrev установлены верно
		if (pCurr->rec.key == key)
			return true;
		//Искомый ключ больше (правее)
		else if (key > pCurr->rec.key)
		{
			pPrev = pCurr;
			pCurr = pCurr->pRight;
		}
		//Искомый ключ меньше (левее)
		else
		{
			pPrev = pCurr;
			pCurr = pCurr->pLeft;
	...
	//pCurr передвигается на pPrev для того, чтобы
	//pCurr указывал на предполагаемое место вставки
	//элемента с ключом key, т.к. таковой ещё не найден
	pCurr = pPrev;
	return false;
}
...
bool TTreeTable::Insert(TRecord rec)
{
	...
	//Добавление первого элемента (самописное)
	if (pRoot == nullptr)
		pRoot = newNode;
...
bool TTreeTable::Delete(TKey key)
{
	...
	//Объявим указатель на звено, которое планируется удалить
